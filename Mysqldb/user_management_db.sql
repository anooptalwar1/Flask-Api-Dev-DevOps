...
-- -----------------------------------------------------
CREATE VIEW `users_for_dashboard_view` AS select `user`.`id` AS `id`,`user`.`name` AS `name`,`user`.`email` AS `email`,`user`.`level` AS `level`,`user`.`created_on` AS `created_on`,`user`.`modified_on` AS `modified_on` from `user`;
